...
sbatch --job-name=ref --array=0-0 \
  --export=ALL,JOB_TYPE=reference,STAGE=reference,CONFIG=src/configs/experiment_config.py \
  run_stage.sbatch

sbatch --job-name=artifact --array=0-0 \
  --export=ALL,JOB_TYPE=condition,STAGE=artifact,CONFIG=src/configs/experiment_config.py \
  run_stage.sbatch

sbatch --job-name=ref --array=0-2 \
  --export=ALL,JOB_TYPE=reference,STAGE=reference,CONFIG=src/configs/experiment_config.py \
  run_stage.sbatch

sbatch --job-name=artifact --array=0-47 \
  --export=ALL,JOB_TYPE=condition,STAGE=artifact,CONFIG=src/configs/experiment_config.py \
  run_stage.sbatch