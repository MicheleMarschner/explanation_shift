...
from src.eval_pipeline.helper import load_clean_ref, load_corr_ref
...
def save_case_row(name: str, i: int, save_path, sal_clean, sal_corr, X_clean, X_corr):
    # images
    img_clean = img_to_hwc01(X_clean[i])
    img_corr  = img_to_hwc01(X_corr[i])

    # explanations (from your loaded refs)
    sal_clean_2d = sal_to_2d(sal_clean[i])
    sal_corr_2d  = sal_to_2d(sal_corr[i])
...
def find_sample_idx(clean_ref, corr_ref):
    ...
    return silent_idx, stubborn_idx, flip_idx



def create_heatmaps(exp_dir, save_dir, corruption="gaussian_noise", severity=5):
    ...
    silent_idx, stubborn_idx, flip_idx = find_sample_idx(clean_ref, corr_ref, corruption, severity)

    ref_pt  = exp_dir.reference / "00__clean_ref.pt"
    ref = torch.load(ref_pt, map_location="cpu")
    pair_idx = ref['pair_idx']

    _, X_clean, _ = get_clean_data(path=PATHS.data_clean, idx=pair_idx, transform=None)
    ...
    save_case_row("SILENT DRIFT",  silent_idx,   save_dir / f"qual_silent_idx{silent_idx}_{corruption}_sev{severity}.png", clean_ref['sal_clean'], corr_ref['sal_corr'], X_clean, X_corr)
    save_case_row("STUBBORN FAIL", stubborn_idx, save_dir / f"qual_stubborn_idx{stubborn_idx}_{corruption}_sev{severity}.png", clean_ref['sal_clean'], corr_ref['sal_corr'], X_clean, X_corr)
    save_case_row("FLIP FAIL",     flip_idx,     save_dir / f"qual_flip_idx{flip_idx}_{corruption}_sev{severity}.png", clean_ref['sal_clean'], corr_ref['sal_corr'], X_clean, X_corr)
